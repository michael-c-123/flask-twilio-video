...
function addPersonToTable(id, panNode) {
    ...
    if (n == 1) {
        ...
        for (let i = 0; i < n; i++) {
            map[i].panNode.pan.value = -1 + step * i;
        }
    }
}
...
function drop(ev) {
    ...
    map[srcIndex].panNode.pan.value = -1 + step * destIndex;
    map[destIndex].panNode.pan.value = -1 + step * srcIndex;
...
function trackSubscribed(div, track, participantID=null) {
    ...
    if(trackElement.tagName === "AUDIO" && participantID != null){
        let audio = trackElement.captureStream();
...
function setPositions() {
    ...
    let step = 2*MAG / (n - 1);
    for (let i = 0; i < n; i++) {
        let y = MAG - Math.sqrt(MAG*MAG - Math.pow(-MAG + i * step, 2));
        console.log(y);
        chairs[i].style.paddingTop = y + 'px';
    }
